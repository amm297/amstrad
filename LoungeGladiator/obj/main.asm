...
; This file was generated Tue Oct 20 12:46:27 2015
;--------------------------------------------------------
	.module main
	.optsdcc -mz80
...
_followPlayer::
	...
	add	hl,sp
	ld	sp,hl
;src/ia.h:55: enemy.dir = setDirection(player.x,player.y,enemy.x,enemy.y);
	...
	ld	hl,#(_enemy + 0x0009)
	ld	(hl),d
;src/ia.h:56: movement(enemy.dir,&enemy.x,&enemy.y);
	ld	hl,#_enemy
	ld	bc,#(_enemy + 0x0001)
	push	bc
	push	hl
	...
	pop	af
	inc	sp
;src/ia.h:57: if(scene[(enemy.y)/tileheight][(enemy.x)/tilewidth] == 1
	...
	dec	a
	jp	Z,00101$
;src/ia.h:58: || scene[(enemy.y)/tileheight][(enemy.x+tilewidth-1)/tilewidth] == 1
	ld	b,#0x00
	ld	hl,#0x0003
	add	hl,bc
	ld	-4 (ix),l
...
00108$:
	...
	dec	a
	jp	Z,00101$
;src/ia.h:59: || scene[(enemy.y+tileheight-2)/tileheight][(enemy.x)/tilewidth] == 1
	ld	e,-8 (ix)
	ld	d,#0x00
	ld	hl,#0x000E
	add	hl,de
...
00109$:
	...
	dec	a
	jr	Z,00101$
;src/ia.h:60: || scene[(enemy.y+tileheight-2)/tileheight][(enemy.x+tilewidth-1)/tilewidth] == 1
	ld	a,-11 (ix)
	or	a, a
	jr	Z,00110$
	ld	c,-6 (ix)
...
00111$:
	...
	jr	NZ,00102$
00101$:
;src/ia.h:62: enemy.x=enemy.lx;
	ld	a, (#_enemy + 2)
	ld	(#_enemy),a
;src/ia.h:63: enemy.y=enemy.ly;
	ld	a, (#_enemy + 3)
	ld	(#(_enemy + 0x0001)),a
00102$:
;src/ia.h:66: return following;
	ld	l,#0x01
	ld	sp, ix
	pop	ix
	ret
;src/ia.h:70: void patrol(){
;	---------------------------------
; Function patrol
; ---------------------------------
_patrol::
	...
	add	hl,sp
	ld	sp,hl
;src/ia.h:71: movement(enemy.dir,&enemy.x,&enemy.y);
	ld	bc,#(_enemy + 0x0001)
	ld	de,#_enemy
	ld	a, (#_enemy + 9)
	push	bc
	...
	pop	af
	inc	sp
;src/ia.h:72: if(scene[(enemy.y)/tileheight][(enemy.x)/tilewidth] != enemy.room
	ld	a,(#(_enemy + 0x0001) + 0)
	ld	-1 (ix), a
	rlca
	rlca
	...
	sub	a, d
	jp	NZ,00101$
;src/ia.h:73: || scene[(enemy.y)/tileheight][(enemy.x+tilewidth-1)/tilewidth] != enemy.room
	ld	e,-7 (ix)
	ld	d,#0x00
	ld	hl,#0x0003
	add	hl,de
...
00108$:
	...
	sub	a,(hl)
	jp	NZ,00101$
;src/ia.h:74: || scene[(enemy.y+tileheight-2)/tileheight][(enemy.x)/tilewidth] != enemy.room
	ld	e,-1 (ix)
	ld	d,#0x00
	ld	hl,#0x000E
	add	hl,de
...
00109$:
	...
	sub	a,(hl)
	jr	NZ,00101$
;src/ia.h:75: || scene[(enemy.y+tileheight-2)/tileheight][(enemy.x+tilewidth-1)/tilewidth] != enemy.room
	ld	a,-3 (ix)
	or	a, a
	jr	Z,00110$
	ld	c,-5 (ix)
...
00111$:
	...
	jr	Z,00106$
00101$:
;src/ia.h:77: enemy.x=enemy.lx;
	ld	a, (#_enemy + 2)
	ld	(#_enemy),a
;src/ia.h:78: enemy.y=enemy.ly;
	ld	a, (#_enemy + 3)
	ld	(#(_enemy + 0x0001)),a
00106$:
	ld	sp, ix
...
_vissionSensor::
	...
	add	hl,sp
	ld	sp,hl
;src/ia.h:83: u8 following = 0;
	ld	-5 (ix),#0x00
;src/ia.h:84: u8 cx = enemy.x/tilewidth;
	ld	a, (#_enemy + 0)
	rrca
	rrca
	and	a,#0x3F
	ld	-4 (ix),a
;src/ia.h:85: u8 cy = enemy.y/tilewidth;
	ld	a, (#(_enemy + 0x0001) + 0)
	rrca
	rrca
	and	a,#0x3F
	ld	c,a
;src/ia.h:86: u8 pcx = player.x/tilewidth;
	ld	a, (#_player + 0)
	rrca
	rrca
	and	a,#0x3F
	ld	-2 (ix),a
;src/ia.h:87: u8 pcy = player.y/tilewidth;
	ld	a, (#(_player + 0x0001) + 0)
	rrca
	rrca
	and	a,#0x3F
	ld	-3 (ix),a
;src/ia.h:90: for(i=0;i<3;i++){
	ld	e,#0x00
00107$:
;src/ia.h:91: lex = cx - i;
	ld	a,-4 (ix)
	sub	a, e
	ld	-6 (ix),a
;src/ia.h:92: ley = cy - i;
	ld	a,c
	sub	a, e
	ld	-1 (ix),a
;src/ia.h:93: mex = cx + i;
	ld	a,-4 (ix)
	add	a, e
	ld	-7 (ix),a
;src/ia.h:94: mey = cy + i;
	ld	a,c
	add	a, e
	ld	d,a
;src/ia.h:95: if(lex == pcx || ley == pcy || mex == pcx || mey == pcy){
	ld	a,-6 (ix)
	sub	a, -2 (ix)
	jr	Z,00101$
	ld	a,-1 (ix)
...
00122$:
	jp	P,00102$
;src/main.c:238: enemy.dir = chooseDirection(enemy.dir);
	ld	hl,#_enemy + 9
	ld	d,(hl)
	push	hl
	push	de
	...
	pop	hl
	ld	(hl),a
;src/main.c:239: following = detectPlayerRoom(player.x,player.y,enemy.room);
	ld	hl, #_enemy + 11
	ld	d,(hl)
	ld	a, (#_player + 1)
	ld	hl, #_player + 0
	...
	ld	iy,#_following
	ld	0 (iy),l
;src/main.c:240: temp = 0;
	ld	hl,#0x0000
	ld	(_temp),hl
00102$:
;src/main.c:242: if(temp%6 == 0){
	ld	hl,#0x0006
	push	hl
	ld	hl,(_temp)
	push	hl
	...
	or	a,l
	jr	NZ,00107$
;src/main.c:243: if(following == 0){
	ld	a,(#_following + 0)
	or	a, a
	jr	NZ,00104$
;src/main.c:244: patrol();
	call	_patrol
	jr	00107$
00104$:
;src/main.c:247: followPlayer();
	call	_followPlayer
00107$:
;src/main.c:250: temp += 1;
	ld	hl, #_temp+0
	inc	(hl)
	ret	NZ
	ld	hl, #_temp+1
...
;src/main.c:257: TNivel n = {0,0,0};
	...
	ld	h,-3 (ix)
	ld	(hl),#0x00
;src/main.c:258: bound =0;
	ld	hl,#_bound + 0
	ld	(hl), #0x00
;src/main.c:259: temp = 0;
	ld	hl,#0x0000
	ld	(_temp),hl
;src/main.c:260: map = 1;
	ld	hl,#_map + 0
	ld	(hl), #0x01
;src/main.c:261: arrow =0;
	ld	hl,#_arrow + 0
	ld	(hl), #0x00
;src/main.c:262: finish =0;
	ld	hl,#_finish + 0
	ld	(hl), #0x00
;src/main.c:263: following =0;
	ld	hl,#_following + 0
	ld	(hl), #0x00
;src/main.c:264: archer = 0;
	ld	hl,#_archer + 0
	ld	(hl), #0x00
;src/main.c:266: initPlayer();
	call	_initPlayer
;src/main.c:267: initEnemies();
	call	_initEnemies
;src/main.c:269: cpct_clearScreen(0);
	ld	hl,#0x4000
	push	hl
	xor	a, a
	push	af
...
;src/main.c:276: while (1){
	...
	ld	-1 (ix),a
00112$:
;src/main.c:279: cpct_waitVSYNC();
	call	_cpct_waitVSYNC
;src/main.c:282: erases();
	call	_erases
;src/main.c:285: drawPickUps(n.corazon,n.bullet);
	ld	l,-4 (ix)
	ld	h,-3 (ix)
	ld	b,(hl)
	ld	l,-6 (ix)
	...
	call	_drawPickUps
	pop	af
;src/main.c:288: draws();
	call	_draws
;src/main.c:291: if(player.atk < 20) drawFatiga(player.atk,2);
	ld	hl, #(_player + 0x0008) + 0
	ld	d,(hl)
	ld	a,d
	sub	a, #0x14
	...
	jr	00106$
00105$:
;src/main.c:292: else if(player.atk > 20) drawFatiga(player.atk,1);
	ld	a,#0x14
	sub	a, d
	jr	NC,00102$
	ld	a,#0x01
	...
	jr	00106$
00102$:
;src/main.c:293: else drawFatiga(player.atk,0);
	xor	a, a
	push	af
	inc	sp
	push	de
	...
	pop	af
00106$:
;src/main.c:296: player.lx = player.x;
	ld	a, (#_player + 0)
	ld	hl,#(_player + 0x0002)
	ld	(hl),a
;src/main.c:297: player.ly = player.y;
	ld	a, (#_player + 1)
	ld	(#(_player + 0x0003)),a
;src/main.c:298: enemy.lx = enemy.x;
	ld	a, (#_enemy + 0)
	ld	(#(_enemy + 0x0002)),a
;src/main.c:299: enemy.ly = enemy.y;
	ld	a, (#_enemy + 1)
	ld	hl,#(_enemy + 0x0003)
	ld	(hl),a
;src/main.c:300: player.latk = player.atk;
	ld	a, (#(_player + 0x0008) + 0)
	ld	(#(_player + 0x0009)),a
;src/main.c:305: cpct_scanKeyboard_f();
	call	_cpct_scanKeyboard_f
;src/main.c:306: player.sprite = checkKeyboard();
	call	_checkKeyboard
	ld	e,l
	ld	d,h
	ld	((_player + 0x0004)), de
;src/main.c:307: checkBoundsCollisions(&n.corazon,&n.bullet);
	ld	e,-8 (ix)
	ld	d,-7 (ix)
	ld	l,-2 (ix)
	ld	h,-1 (ix)
...
;src/main.c:309: move();
	call	_move
;src/main.c:311: if(arrow == 1){
	ld	a,(#_arrow + 0)
	dec	a
	jr	NZ,00108$
;src/main.c:312: moveObject();
	call	_moveObject
;src/main.c:313: bound = checkArrowCollisions();
	call	_checkArrowCollisions
	ld	iy,#_bound
	ld	0 (iy),l
00108$:
;src/main.c:318: if(finish == 1) return;
	ld	a,(#_finish + 0)
	dec	a
	jp	NZ,00112$
	ld	sp, ix
	pop	ix
	ret
;src/main.c:325: void credits(){
;	---------------------------------
; Function credits
; ---------------------------------
_credits::
;src/main.c:327: cpct_clearScreen(0);
	ld	hl,#0x4000
	push	hl
	xor	a, a
	push	af
	...
	push	hl
	call	_cpct_memset
;src/main.c:328: memptr = cpct_getScreenPtr(VMEM,10,10);
	ld	hl,#0x0A0A
	push	hl
	ld	hl,#0xC000
	push	hl
	call	_cpct_getScreenPtr
;src/main.c:329: cpct_drawStringM0("Lounge Gladiator",memptr,1,0);
	ex	de,hl
	ld	bc,#___str_5
	push	de
	ld	hl,#0x0001
	...
	ld	sp,hl
	pop	de
;src/main.c:331: while (1){
00104$:
;src/main.c:333: cpct_scanKeyboard_f();
	push	de
	call	_cpct_scanKeyboard_f
	pop	de
;src/main.c:334: cpct_drawStringM0("Josep Domenech Mingot",memptr,1,0);
	ld	bc,#___str_6
	push	de
	ld	hl,#0x0001
	push	hl
	...
	ld	sp,hl
	pop	de
;src/main.c:335: cpct_drawStringM0("Alberto Martinez Martinez",memptr,1,0);
	ld	bc,#___str_7
	push	de
	ld	hl,#0x0001
	push	hl
...
00106$:
;src/main.c:357: x=menu();
	call	_menu
	ld	e, l
	ld	d, h
;src/main.c:358: switch(x){
	bit	7, d
	jr	NZ,00106$
	ld	a,#0x02
	cp	a, e
...
00122$:
	...
	add	hl,de
	add	hl,de
;src/main.c:359: case 0: return;break;
;src/main.c:360: case 1: game(); break;
	jp	(hl)
00123$:
	jr	00108$
	jr	00102$
...
00102$:
	call	_game
	jr	00106$
;src/main.c:361: case 2: credits();break;
00103$:
	call	_credits
;src/main.c:362: }
	jr	00106$
